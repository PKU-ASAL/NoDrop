...
// 	const enum nod_syscall_code *cur_g_syscall_code_routing_table = args->cur_g_syscall_code_routing_table;
...
// 		enum nod_syscall_code sc_code = cur_g_syscall_code_routing_table[table_index];
...
// 		if (args->event_type == NOD_GENERIC_E) {
// 			/*
// 			 * nativeID
// 			 */
				  ...
				  int available)
{
	...
	for (;;) {
		...
		if (unlikely(nod_get_user(compat_p, compat_ptr(argv))))
			return NOD_FAILURE_INVALID_USER_MEMORY;
...
int f_proc_startupdate(struct event_filler_arguments *args)
{
	...
	if (unlikely(retval < 0 &&
		     args->event_type != SPRE_SYSCALL_EXECVE_19_X &&
			 args->event_type != SPRE_SYSCALL_EXECVEAT_X)) {
		...
		*args->str_storage = 0;
		...
		if (likely(retval >= 0)) {
			...
			{
			case SPRE_SYSCALL_EXECVE_19_X:
				syscall_get_arguments_deprecated(current, args->regs, 1, 1, &val);
				break;
			
			case SPRE_SYSCALL_EXECVEAT_X:
				syscall_get_arguments_deprecated(current, args->regs, 2, 1, &val);
				break;
...
cgroups_error:
	...
	if (args->event_type == SPRE_SYSCALL_CLONE_20_X ||
		args->event_type == SPRE_SYSCALL_FORK_20_X ||
		args->event_type == SPRE_SYSCALL_VFORK_20_X ||
		args->event_type == SPRE_SYSCALL_CLONE3_X) 
		{
		/*
		 * clone-only parameters
		 */
	...
	} else if (args->event_type == SPRE_SYSCALL_EXECVE_19_X || 
			   args->event_type == SPRE_SYSCALL_EXECVEAT_X) {
		/*
...
int f_sys_getrlimit_setrlrimit_x(struct event_filler_arguments *args)
{
	 ...
	 */
	if (retval >= 0 || args->event_type == SPRE_SYSCALL_SETRLIMIT_X) {
		syscall_get_arguments_deprecated(current, args->regs, 1, 1, &val);
